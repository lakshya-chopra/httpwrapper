...
func NewHttp2Server(bindAddr string, preMasterSecretLogPath string, handler http.Handler) (*http.Server, error) {
	...
	// Optional: only applies to idle conn reuse, not per-request timeout
	...
	server := &http.Server{
		...
		ReadTimeout:  30 * time.Second, // ⬅️ time to read request headers
		WriteTimeout: 60 * time.Second, // ⬅️ total time to write response
